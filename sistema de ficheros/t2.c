...
int main()
{
	...
	//char *buffer="ldkdkdkkd";
	int aux =writeFile(ret,buffer,strlen(buffer)+1);
	if (aux == -1)
	{
	...
	aux = readFile(ret,(void *)buffer2,strlen(buffer));
	printf("aux leidos: %d\n", aux);
	if (aux == -1)
	{
...
////
	...
	//////
	aux = includeIntegrity("/patata.txt");
		if (aux == -1)
	{
	...
	if (aux == -2)
	{
		fprintf(stdout, "%s%s%s%s%s", ANSI_COLOR_BLUE, "TEST includeIntegrity ", ANSI_COLOR_RED, "FAILED 2\n", ANSI_COLOR_RESET);
		return -1;
	}
	...
	if (aux == -2)
	{
		fprintf(stdout, "%s%s%s%s%s", ANSI_COLOR_BLUE, "TEST checkFile ", ANSI_COLOR_RED, "FAILED 2\n", ANSI_COLOR_RESET);
		return -1;
	}
	...
	aux = openFileIntegrity("/patata.txt");
	if (aux == -1)
	{
	...
	if (aux == -2)
	{
		fprintf(stdout, "%s%s%s%s%s", ANSI_COLOR_BLUE, "TEST openFileIntegrity ", ANSI_COLOR_RED, "FAILED 2\n", ANSI_COLOR_RESET);
		return -1;
	}
	fprintf(stdout, "%s%s%s%s%s", ANSI_COLOR_BLUE, "TEST openFileIntegrity ", ANSI_COLOR_GREEN, "SUCCESS\n", ANSI_COLOR_RESET);

	aux = closeFileIntegrity(aux);
	if (aux == -1)
	{
	...
	if (aux == -2)
	{
		fprintf(stdout, "%s%s%s%s%s", ANSI_COLOR_BLUE, "TEST closeFileIntegrity ", ANSI_COLOR_RED, "FAILED 2\n", ANSI_COLOR_RESET);
		return -1;
	}
