...
int mythread_create (void (*fun_addr)(),int priority,int seconds)
{
  ...
  TCB *t= &t_state[i];
  enqueue(lista_procesos_listos,t);
...
TCB* scheduler()
{ 
  if(lista_procesos_listos!=NULL){
    TCB *p=dequeue(lista_procesos_listos);
...
void timer_interrupt(int sig)//PARA QUE SIRVE SIG= signal
{
  ...
  if(running->ticks==0){
    ...
    enqueue(lista_procesos_listos,running);
    enable_interrupt();
    ...
    //comprobar si el hilo se ha pasado de su tiempo y hacer mythread_timeout
